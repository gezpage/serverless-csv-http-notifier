...
records_data = [
    {"s3": {"bucket": {"name": "some-bucket"}, "object": {"key": "testdata.csv"}}}
]
...
class NotifierTest(unittest.TestCase):
    ...
    def test_processing_valid_csv_data_and_notifying_endpoint(self, m, read_ascii_file):
        ...
        self.assertEqual(
            '{"First name": "Dave", "Last name": "Banks", "Email": "davebanks@email.com"}',
            requests[0].text,
        )
        self.assertEqual(
            '{"First name": "Grant", "Last name": "Davis", "Email": "grantdavis@email.com"}',
            requests[1].text,
        )
